...
struct ImGuiInputTextState;         // MakeInternal state of the currently focused/edited text input box
struct ImGuiLastItemDataBackup;     // Backup and restore IsItemHovered() MakeInternal data
...
{
    ImVector<ImDrawList*>   Layers[2];           // MakeGlobal layers for: regular, tooltip
...
// MakeInternal state of the currently focused/edited text input box
...
struct ImGuiContext
{
    ...
    ImGuiWindow*            NavWindowingListWindow;             // MakeInternal window actually listing the CTRL+Tab contents
...
namespace ImGui
{
    ...
    IMGUI_API void          LogToBuffer(int auto_open_depth = -1);                      // Start logging/capturing to MakeInternal buffer
    ...
    // MakeInternal Columns API (this is not exposed because we will encourage transitioning to the Tables API)
...
#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
    // [1.71: 2019/06/07: Updating prototypes of some of the MakeInternal functions. Leaving those for reference for a short while]
