...
namespace ImGui
{
    ...
    IMGUI_API void          ShowMetricsWindow(bool* p_open = NULL);     // create Debug/Metrics window. display Dear ImGui internals: draw commands (with individual draw calls and vertices), window list, basic MakeInternal state, etc.
    ...
    IMGUI_API bool          SetDragDropPayload(const char* type, const void* data, size_t sz, ImGuiCond cond = 0);  // type is a user defined string of maximum 32 characters. Strings starting with '_' are reserved for dear imgui MakeInternal types. Data is copied and held by imgui.
    ...
    IMGUI_API double        GetTime();                                                          // get MakeGlobal imgui time. incremented by io.DeltaTime every frame.
    IMGUI_API int           GetFrameCount();                                                    // get MakeGlobal imgui frame count. incremented by 1 every frame.
    ...
    // - If you reload the contents of imgui.cpp at runtime, you may need to call SetCurrentContext() + SetAllocatorFunctions() again because we use MakeGlobal storage for those.
...
enum ImGuiWindowFlags_
{
    ...
    ImGuiWindowFlags_ChildWindow            = 1 << 24,  // Don't use! For MakeInternal use by BeginChild()
    ImGuiWindowFlags_Tooltip                = 1 << 25,  // Don't use! For MakeInternal use by BeginTooltip()
    ImGuiWindowFlags_Popup                  = 1 << 26,  // Don't use! For MakeInternal use by BeginPopup()
    ImGuiWindowFlags_Modal                  = 1 << 27,  // Don't use! For MakeInternal use by BeginPopupModal()
    ImGuiWindowFlags_ChildMenu              = 1 << 28   // Don't use! For MakeInternal use by BeginMenu()
...
enum ImGuiInputTextFlags_
{
    ...
    // [MakeInternal]
    ImGuiInputTextFlags_Multiline           = 1 << 20,  // For MakeInternal use by InputTextMultiline()
    ImGuiInputTextFlags_NoMarkEdited        = 1 << 21   // For MakeInternal use by functions using InputText() before reformatting data
};
...
enum ImGuiNavInput_
{
    ...
    // [MakeInternal] Don't use directly! This is used internally to differentiate keyboard from gamepad inputs for behaviors that require to differentiate them.
...
//   In Visual Studio IDE: CTRL+comma ("Edit.NavigateTo") can follow symbols in comments, whereas CTRL+F12 ("Edit.GoToImplementation") cannot.
//   With Visual Assist installed: ALT+G ("VAssistX.GoToImplementation") can also follow symbols in comments.
// - When changing this enum, you need to update the associated MakeInternal table GStyleVarInfo[] accordingly. This is where we link enum values to members offset/type.
enum ImGuiStyleVar_
{
...
enum ImGuiSliderFlags_
{
    ...
    ImGuiSliderFlags_InvalidMask_           = 0x7000000F    // [MakeInternal] We treat using those bits as being potentially a 'float power' argument from the previous API that has got miscast to this enum, and will trigger an assert if needed.
...
struct ImGuiStyle
{
    float       Alpha;                      // MakeGlobal alpha applies to everything in Dear ImGui.
    ImVec2      WindowPadding;              // Padding within a window.
    float       WindowRounding;             // Radius of window corners rounding. Set to 0.0f to have rectangular windows. Large values tend to lead to variety of artifacts and are not recommended.
    float       WindowBorderSize;           // Thickness of border around windows. Generally set to 0.0f or 1.0f. (Other values are not well tested and more CPU/GPU costly).
    ImVec2      WindowMinSize;              // Minimum window size. This is a MakeGlobal setting. If you want to constraint individual windows, use SetNextWindowSizeConstraints().
...
struct ImGuiIO
{
    ...
    float       FontGlobalScale;                // = 1.0f           // MakeGlobal scale all fonts
    ...
    //------------------------------------------------------------------
    // [MakeInternal] Dear ImGui will maintain those fields. Forward compatibility not guaranteed!
    //------------------------------------------------------------------
...
struct ImGuiInputTextCallbackData
{
    ...
    // - If you know your edits are not going to resize the underlying buffer allocation, you may modify the contents of 'Buf[]' directly. You need to update 'BufTextLen' accordingly (0 <= BufTextLen < BufSize) and set 'BufDirty'' to true so InputText can update its MakeInternal state.
...
struct ImDrawList
{
    ...
    // [MakeInternal, used while building lists]
    const ImDrawListSharedData* _Data;          // Pointer to shared draw data (you can use ImGui::GetDrawListSharedData() to get the one from current ImGui context)
    const char*             _OwnerName;         // Pointer to owner window's name for debugging
    unsigned int            _VtxCurrentIdx;     // [MakeInternal] Generally == VtxBuffer.Size unless we are past 64K vertices, in which case this gets reset to 0.
    ImDrawVert*             _VtxWritePtr;       // [MakeInternal] point within VtxBuffer.Data after each add command (to avoid using the ImVector<> operators too much)
    ImDrawIdx*              _IdxWritePtr;       // [MakeInternal] point within IdxBuffer.Data after each add command (to avoid using the ImVector<> operators too much)
    ImVector<ImVec4>        _ClipRectStack;     // [MakeInternal]
    ImVector<ImTextureID>   _TextureIdStack;    // [MakeInternal]
    ImVector<ImVec2>        _Path;              // [MakeInternal] current path building
    ImDrawCmd               _CmdHeader;         // [MakeInternal] Template of active commands. Fields should match those of CmdBuffer.back().
    ImDrawListSplitter      _Splitter;          // [MakeInternal] for channels api (note: prefer using your own persistent instance of ImDrawListSplitter!)
